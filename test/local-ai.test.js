...
test("local layouts are cleaned before the dashboard can build them", () => {
  ...
  assert.deepEqual(community.channels, [
...
test("dashboard image bytes reach the local vision model", async () => {
  ...
  await localPlan({
    ...base,
    screenshot: { data, size: 12, contentType: "image/png" },
    fetchFn: async (url, init) => {
...
test("larger layouts preserve requested channel types and emoji", () => {
  ...
  assert.equal(
    plan.categories
      ...
      .filter((c) => c.name.startsWith("channel-")).length,
    35,
  );
  assert.ok(
    plan.categories
      .flatMap((c) => c.channels)
      .some((c) => c.type === "forum" && c.name === "🎨-art"),
  );
});
